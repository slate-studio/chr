...
#   viewTabs         - hash with group input names and titles
#
# Callbacks:
#   onViewShow    - on show callback
#   onNewShow     - on new object view show callback
#   onEditShow    - on edit object view show callback
#   onSaveSuccess - on document succesfully saved callback
#   onSaveError   - on document save error callback
#
# Methods:
#   show(objectId)
#   destroy()
...
class @View
  ...
  _save_success: ->
    @$el.removeClass("view-new")
    @$el.addClass("view-edit")
  ...
  _save_error: (message, validationErrors) ->
    ...
    @config.onSaveError?(@, validationErrors)
    ...
    if @object
      @store.update @object._id, serializedFormObj,
        onSuccess: (@object) =>
          @_save_success()
        onError: (errors) =>
          @_save_error('Changes are not saved.', errors)
    else
